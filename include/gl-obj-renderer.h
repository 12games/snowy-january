...
namespace MaskedTexturesBuffer
{
    ...
    public:
        ShaderType()
            : _shaderId(0),
              ...
              _textureUniform1Name("u_texture1"),
              _textureUniform2Name("u_texture2"),
              _textureUniform3Name("u_texture3"),
              _textureUniformMaskName("u_mask"),
              _vertexAttributeName("vertex"),
              _colorAttributeName("color"),
              _uvsAttributeName("uvs")
        {}
            ...
            std::string const &fragShaderFile)
        {
            ...
            std::string vertShaderStr(
                (std::istreambuf_iterator<char>(vertShaderFileStream)),
                std::istreambuf_iterator<char>());

            std::ifstream fragShaderFileStream(fragShaderFile.c_str());

            std::string fragShaderStr(
                (std::istreambuf_iterator<char>(fragShaderFileStream)),
                std::istreambuf_iterator<char>());
        ...
        void setupAttributes() const
        {
            ...
            glVertexAttribPointer(
                GLuint(uvsAttrib),
                sizeof(VertexType::uvs) / sizeof(float),
                GL_FLOAT,
                GL_FALSE,
                vertexSize,
    ...
    public:
        ...
        {
            _verts.push_back(
                VertexType({position, _nextColor, _nextUvs}));
